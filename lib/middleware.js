module.exports = function middlewareConstructor(env) {
  ...
  return {
    ...
    ensureMetaData: function(req, res, next) {
      ...
      // replace username placeholder in the title only.
      req.body.html = req.body.html.replace(/(<title>{{username}})/, function(a) {
        return a.replace("{{username}}", req.session.user.username);
      });

      // extract title
      var title_match = req.body.html.match(/<title[^>]*?>([^<]*?)<\/title>/);
      var title = title_match && title_match[1] ? title_match[1] : req.body["original-url"];

      req.body.metaData.title = title;
      req.body.metaData.description = "An X-Ray Goggles remix of " + title;
    ...
    finalizeProject: function(nunjucksEnv, env) {
      var hostname = env.get("APP_HOSTNAME");
      return function(req, res, next) {
    ...
    publishMake: function(make) {
      return function(req, res, next) {
        ...
        function finalizePublishMake(err) {
          console.log("running finalizePublishMake");

          if (err) {
            console.log("And error was registered in finalizePublishMake:", JSON.stringify(err,false,2));
            metrics.increment('makeapi.publish.error');
            return next(err);
          }

          console.log("No errors were registered in finalizePublishMake");
          metrics.increment('makeapi.publish.success');

          console.log("Handing control to next middleware function");
          next();
        }
        ...
        if (project.makeid) {
          ...
          make.search({
            ...
            if (result) {
              ...
              console.log("updating an existing make (this should not happen with goggles!)");

              console.log("Preparing to update make attributes, using make id " + result.id);
              project.updateAttributes({ makeid: result.id })
              .error(function(err) {
                console.log("An error occurred in project.updateAttributes... JSON printout:", JSON.stringify(err,false,2));
                return next(err);
              })
              .success(function(updatedProject) {
                console.log("attribute updating succeeded, binding req.project");
                req.project = updatedProject;

                console.log("preparing to propagate the update via the makeapi...");
                make.update(updatedProject.makeid, options, finalizePublishMake);
              });
            } else {

              console.log("creating a new make");

              make.create(options, function( err, make ) {
                if (err) {
                  console.log("an error occurred inside make.create... JSON printout:", JSON.stringify(err,false,2));
                  return finalizePublishMake(err);
                }

                if (!make) {
                  console.log("no make was generated during make.create!");
                  return finalizePublishMake(new Error("Make failed to create"));
                }

                console.log("Preparing to update make attributes, using make id " + make.id);
