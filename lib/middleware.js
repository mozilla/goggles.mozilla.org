  ...
  return {
    ...
    finalizeProject: function(nunjucksEnv, env) {
      var hostname = env.get("HOSTNAME"),
          projectURL = hostname + "/project/" + req.publishId,
      return function(req, res, next) {
              ...
              s3.put(location, headers)
                ...
                .on("response", function(res) {
                  if (res.statusCode === 200) {
                    ...
                    var target = req.body.projectURL;
      ...
      return function(req, res, next) {
        var metaData = req.body.metaData,
            options = {
              ...
              url: req.publishedUrl,
              remixedFrom: req.body.remixedFrom,
              // note: There is no remixing involved here,
              //       you just fire up the goggles if you want.
              remixUrl: req.body.projectURL,
