(function(jQuery) {
  ...
  function addHTTPSWarning(hud) {
    var warning = $('<div class="webxray-base webxray-https-warning"></div>');
    warning.text(Localized.get("https-warning"));
    warning.click(function() { warning.remove(); });
    $(hud.overlayContainer).append(warning);
  }
    ...
    xRayUI: function xRayUI(options) {
      ...
      var self = jQuery.eventEmitter({
        ...
        start: function() {
          ...
          // Check whether we're running on http://, because if
          // we are, we need to warn about potential CORS issues.
